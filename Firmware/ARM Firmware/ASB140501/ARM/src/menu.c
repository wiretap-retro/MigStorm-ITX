...
// 2014-04-30   - added load of alternative FPGA core .BIN file
// 2014-05-01   - added chipset selection via keyboard shortcut
...
const char *config_filter_msg[] =  {"Disabled", "Hor", "Ver", "Hor+Ver"};
const char *config_memory_chip_msg[] = {"0.5 MB", "1.0 MB", "1.5 MB", "2.0 MB"};
const char *config_memory_slow_msg[] = {"0.0 MB", "0.5 MB", "1.0 MB", "1.5 MB"};
const char *config_scanlines_msg[] = {"VGA alike", "TV  alike", "Cache Off"};

char *config_chipset_msg[] = {"OCS-A500", "OCS-A1000", "ECS", ""};
...
void HandleUI(void)
{
    ...
    case KEY_KP0 :
        ...
        break;
    case KEY_HOME : // chipset selection via keyboard shortcut
        if (ctrl && lalt)
        {
    ...
    case MENU_MAIN2_1 :
        ...
        OsdWrite(5, "            FPGA Core", menusub == 3);
        OsdWrite(6, "", 0);
        OsdWrite(7, "              Exit", menusub == 4);
    ...
    case MENU_MAIN2_2 :
        ...
        else if (select)
        {
            ...
            else if (menusub == 3) // FPGA core file select
            {
                menusub = 3;
                SelectFile("BIN", SCAN_LFN, MENU_COREFILE_SELECTED, MENU_MAIN2_1);
            }
            else if (menusub == 4)
            {
                menusub = 0;
        ...
        /******************************************************************/
        /* core file selected menu                                         */
        /******************************************************************/
    case MENU_COREFILE_SELECTED :

        menusub = 1;

    case MENU_COREFILE_SELECTED1 :

        OsdWrite(0, "", 0);
        OsdWrite(1, "        Upload FPGA Core?", 0);
        OsdWrite(2, "", 0);
        ...
        menustate = MENU_COREFILE_SELECTED2;
        break;

    case MENU_COREFILE_SELECTED2 :

        if (down && menusub < 1)
        {
            menusub++;
            menustate = MENU_COREFILE_SELECTED1;
        }
        ...
        if (select && menusub == 0)
        {
            memcpy((void*)config.corefile.name, (void*)file.name, sizeof(config.corefile.name));
            strcpy(&config.corefile.name[8], "BIN"); // add extension
            ConfigureFpga(config.corefile.name);
        }
        else if ((select && menusub == 1) || menu)
        {
            menusub = 3;
            menustate = MENU_MAIN2_1;
        }
    ...
    case MENU_FILE_SELECT2 :
        ...
        if (c == KEY_PGUP || left) // scroll up one page
        {
            ScanDirectory(SCAN_PREV_PAGE, fs_pFileExt, fs_Options);
            menustate = MENU_FILE_SELECT1;
        }

        if (c == KEY_PGDN || right) // scroll down one pge
        {
    ...
    case MENU_SETTINGS_DRIVES1 :
        ...
        OsdWrite(3, s, menusub == 1);
        strcpy(s,  "       A600 IDE : ");
        strcat(s, config.enable_ide ? "Enabled" : "Disabled");
        OsdWrite(4, s, menusub == 2);
    ...
    case MENU_SETTINGS_HARDFILE1 :
        ...
        OsdWrite(1, "", 0);
        strcpy(s, "     Master : ");
        strcat(s, config.hardfile[0].present ? config.hardfile[0].enabled ? "Enabled" : "Disabled" : "Undefined");
        OsdWrite(2, s, menusub == 0);
        if (config.hardfile[0].present)
        {
            ...
            OsdWrite(3, "              * no file *", menusub == 1);

        strcpy(s, "     Slave  : ");
        strcat(s, config.hardfile[1].present ? config.hardfile[1].enabled ? "Enabled" : "Disabled" : "Undefined");
        OsdWrite(4, s, menusub == 2);
...
//         ConfigureFpga("MINIMIG2BIN");
    ...
    case MENU_FIRMWARE1 :
        ...
        OsdWrite(1, "", 0);
        sprintf(s, "   Running ARM fw: %s", version + 5);
        OsdWrite(2, s, 0);
        OsdWrite(3, "", 0);
        OsdWrite(4, "            SD-Update", menusub == 0);
    ...
    case MENU_FIRMWARE_UPDATE_ERROR1 :
        ...
        case ERROR_UPDATE_FAILED :
            OsdWrite(1, "         Update failed!", 0);
            OsdWrite(2, "    Dont panic! Flash ARM via", 0);
            OsdWrite(3, "    USB and SAM-BA/PROG Tool!", 0);
            break;
        }
    ...
    case MENU_FIRMWARE_OPTIONS1 :
        ...
        {
            OsdWrite(2, "      Fast SPI is enabled", menusub == 0);

            OsdWrite(4, "    Hold MENU button while", 0);
            OsdWrite(5, "    powering-on to disable it.", 0);
        }
        else
...
// it checks if predefined period of time has elapsed and scrolls the name if necessary
    ...
    unsigned short  k = sort_table[iSelectedEntry];
